...
export default function BoardAdmin({
  ...
  async function addPositionOption(label: string): Promise<string | null> {
    const result = await adminFetch<{ label?: string }>(
      "/api/admin/position-options",
      {
        method: "POST",
    ...
    setPositionOptions((opts) =>
      opts.includes(added) ? opts : [...opts, added].sort(),
    );
    return null;
  }
  ...
  async function savePosition(m: BoardMember, nextPosition: string) {
    ...
    setBoardMembers((rows) =>
      rows.map((r) =>
        r.id === m.id ? { ...r, position: trimmed || null } : r,
      ),
    );
  ...
  async function saveDetail(
    ...
    setBoardMembers((rows) =>
      rows.map((r) => (r.id === m.id ? { ...r, ...result.data } : r)),
    );
  ...
  async function remove(m: BoardMember) {
    const ok = await confirm(
      `Remove ${m.name}'s (${m.email}) CMS login? This cannot be undone.`,
    );
    if (!ok) return;
    setError("");
    const result = await adminFetch(`/api/admin/board/${m.id}`, {
  ...
  return (
    <div>
      ...
      <p className="admin-note">
        People listed here can log in to this admin site. Adding someone or
        sending a reset link emails them a one-time link to set their own
        password — nobody else ever sees it. Presidents can add and manage
        regular board members and other presidents; only a tech admin can create
        or change a tech admin login. Title (Vice President, Treasurer, etc.) is
        just a label — it doesn't change what someone can do here, only their
        access level does.
      </p>
                  ...
                  <td data-label="Access">
                    {canManage ? (
                      <select
                        value={m.role}
                        onChange={(e) =>
                          setMemberRole(m, e.target.value as Role)
                        }
                      >
